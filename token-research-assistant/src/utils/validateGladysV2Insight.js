...
function normalizePriorityAngles(value) {
...
function inferDeterministicOutcome(comparison) {
    ...
    if (!stronger || stronger === 'Neither side')
        return 'tie';
    if (stronger === leftName)
        return 'left';
    if (stronger === rightName)
        return 'right';
    return 'tie';
}
function findMetricWinner(comparison, key) {
    return comparison.comparativeIntelligence?.items.find((item) => item.key === key)?.betterSide ?? 'unknown';
}
export function validateGladysV2Insight(candidate, comparison) {
