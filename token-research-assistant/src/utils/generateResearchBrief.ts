...
type ProjectData = Pick<ResearchResult['project'], 'description' | 'categories'> & { name?: string | null };
...
function buildWhatItIs(project: ProjectData) {
...
function buildMarketPosture(market: MarketData, risk: RiskAnalysis) {
  const largeMarketCap = market.marketCapUsd !== null && market.marketCapUsd >= 1_000_000_000;
  const mediumMarketCap = market.marketCapUsd !== null && market.marketCapUsd >= 100_000_000 && market.marketCapUsd < 1_000_000_000;
  const highLiquidity = market.volume24hUsd !== null && market.volume24hUsd >= 10_000_000;
  const mediumLiquidity = market.volume24hUsd !== null && market.volume24hUsd >= 1_000_000 && market.volume24hUsd < 10_000_000;
  const lowLiquidity = market.volume24hUsd !== null && market.volume24hUsd < 1_000_000;
  ...
  if (lowLiquidity || (!largeMarketCap && market.marketCapUsd !== null && market.marketCapUsd < 100_000_000) || risk.level === 'high') {
...
function buildWatchout(market: MarketData, risk: RiskAnalysis, signalInterpretation: SignalInterpretation) {
