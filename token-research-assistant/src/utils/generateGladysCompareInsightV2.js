...
function getGladysV2Provider() {
    const explicit = getEnvValue('GLADYS_V2_PROVIDER')?.trim().toLowerCase();
    if (explicit === 'disabled')
        return 'disabled';
    if (explicit === 'remote-proxy')
        return 'remote-proxy';
    if (explicit === 'openai-api-key')
        return 'openai-api-key';
    if (getEnvValue('GLADYS_V2_PROXY_URL')?.trim())
        return 'remote-proxy';
    if (getEnvValue('OPENAI_API_KEY')?.trim())
        return 'openai-api-key';
    return 'disabled';
}
...
function parseGladysV2Response(text) {
    const normalized = stripCodeFences(text);
    return JSON.parse(normalized);
}
async function callOpenAiApiKeyProvider(comparison) {
...
async function callRemoteProxyProvider(comparison) {
    ...
    const json = (await response.json());
    const content = [json.content, json.output, json.result].find((value) => typeof value === 'string' && value.trim().length > 0);
    if (content) {
        return content.trim();
    }
...
async function callGladysV2Provider(comparison) {
