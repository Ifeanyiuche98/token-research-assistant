import type { ResearchResult, RiskAnalysis, RiskBand, RiskDriver, RiskLevel, RiskSignal, RiskSummaryMode } from '../types/research';
...
function createSignal(key: RiskSignal['key'], label: string, value: string, impact: RiskSignal['impact']): RiskSignal {
...
function deriveSummaryMode(level: RiskLevel, market: MarketData): RiskSummaryMode {
  ...
  const hasModestLiquidity = market.volume24hUsd !== null && market.volume24hUsd < 10_000_000;
...
function deriveDominantDriver(market: MarketData, signals: RiskSignal[]): RiskDriver {
  if (signals.some((signal) => signal.key === 'missing_market_data')) return 'missing_data';

  const weights: Record<Exclude<RiskDriver, null | 'trust' | 'honeypot' | 'mixed' | 'missing_data'>, number> = {
    liquidity: 0,
    volatility: 0,
    fdv_gap: 0,
    scale: 0
  ...
  signals.forEach((signal) => {
    ...
    if (signal.key === 'low_volume') weights.liquidity += weight;
    if (signal.key === 'high_24h_move') weights.volatility += weight;
    if (signal.key === 'fdv_gap') weights.fdv_gap += weight;
    if (signal.key === 'small_market_cap') weights.scale += weight;
  });
  ...
  const [topKey, topValue] = ranked[0] as [RiskDriver, number];
...
function buildSummary(level: RiskLevel, dominantDriver: RiskDriver) {
  if (level === 'unknown') {
...
export function calculateRiskAnalysis(market: MarketData): RiskAnalysis {
  ...
  if (missingCoreFields > 0) {
    ...
    signals.push(
      createSignal(
        ...
        `${missingCoreFields} core field${missingCoreFields === 1 ? '' : 's'} missing`,
        'medium'
      )
    );
  }
  ...
  if (market.volume24hUsd !== null) {
    if (market.volume24hUsd < 1_000_000) {
      score += 2.5;
      signals.push(createSignal('low_volume', '24h volume', `< $${formatCompactUsd(1_000_000)}`, 'high'));
