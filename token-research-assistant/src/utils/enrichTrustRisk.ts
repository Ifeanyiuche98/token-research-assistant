...
function createRiskSignal(key: RiskSignal['key'], label: string, value: string, impact: RiskSignal['impact']): RiskSignal {
  return { key, label, value, impact };
}

function createInterpretedSignal(key: InterpretedSignal['key'], label: string, detail: string, tone: SignalTone): InterpretedSignal {
...
async function fetchHoneypot(address: string): Promise<{ honeypot: boolean | null; buyTax: number | null; sellTax: number | null }> {
  try {
    ...
    const data = (await response.json()) as HoneypotResponse;
    return {
...
function getLiquidityRisk(liquidityUsd: number | null): TrustRiskBand {
  if (liquidityUsd === null) return null;
  if (liquidityUsd < 10_000) return 'high';
  if (liquidityUsd <= 100_000) return 'medium';
  return 'low';
}

function getAgeRisk(assetCreatedAt: string | null | undefined): TrustRiskBand {
  if (!assetCreatedAt) return null;
...
function getVolumeAnomaly(volume24hUsd: number | null, liquidityUsd: number | null) {
...
export async function enrichTrustRisk(response: ResearchResponse): Promise<ResearchResponse> {
  ...
  const baseSignalInterpretation: SignalInterpretation = response.result.signalInterpretation ?? {
