...
function buildHeadline(input: GladysInsightInput) {
  if (input.risk.overrideReason === 'honeypot_exit_risk') return 'GLADYS: Severe exit-risk warning';
  if (input.risk.level === 'high') return 'GLADYS: This setup looks structurally fragile';
  if (input.token.source === 'dexscreener' && input.risk.band === 'lower') return 'GLADYS: Calm visible setup, limited trust';
  if (input.risk.level === 'medium') return 'GLADYS: Constructive in parts, but caution still leads';
  if (input.risk.level === 'low') return 'GLADYS: Broadly stable setup';
...
function buildRiskCall(input: GladysInsightInput) {
  if (input.risk.overrideReason === 'honeypot_exit_risk') return 'Main risk call: contract-level exit risk dominates everything else.';
  if (input.risk.overrideReason === 'thin_liquidity_weak_visibility') return 'Main risk call: thin liquidity and weak trust visibility make this setup hard to rely on.';
